// Blueprint 1: parameterized version of the existing PymeWebPro landing page.
// Sections are wrapped with data-section attributes so the comment widget can anchor to them.
...
export interface BlueprintInput {
  ...
  heroImageUrl?: string;
  galleryUrls?: string[];
  ctaPhone?: string;            // for tel: link
  ctaWhatsapp?: string;         // for wa.me link
  language?: "es" | "en";
}

const fallback: BlueprintInput = {
...
export function renderBlueprint1(input: Partial<BlueprintInput>): string {
  const i: BlueprintInput = { ...fallback, ...input };
  ...
  const waLink = i.ctaWhatsapp ? `https://wa.me/${i.ctaWhatsapp.replace(/[^\d]/g, "")}` : "#contacto";
...
${gallery.length ? `<section class="section wrap" data-section="gallery">
  <h2>${t.gallery}</h2>
  <div class="gallery">
    ${gallery.map((u) => `<img src="${esc(u)}" alt="">`).join("")}
  </div>
</section>` : ""}
