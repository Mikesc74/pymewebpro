...
| **plan mensual (opcional)** | No à la carte add-ons anymore: all paid extras live inside ONE optional monthly plan (after the included first month, no contract, cancel anytime, includes hosting). **Plan mensual $150.000 COP/mes**, todo incluido: hosting + Ficha de Google activa y actualizada + versión bilingüe ES/EN + soporte WhatsApp + hasta 2 cambios al mes + asistente de ventas 24/7 (chatbot que conoce el negocio, responde, pasa al WhatsApp con un clic y agenda llamadas) + vitrina de hasta 30 productos + catálogo o menú descargable + reporte mensual de actividad. The base page already includes the Ficha de Google (NOT an add-on). |
