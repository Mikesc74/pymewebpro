...
(function(){
  ...
  // Cache the result in localStorage for 24h to avoid repeated trace calls.
  var html = document.documentElement;
  var cached = null;
  try {
    var raw = localStorage.getItem('pwp-geo');
    if (raw) {
      var parsed = JSON.parse(raw);
      if (parsed && parsed.t && (Date.now() - parsed.t) < 86400000) cached = parsed.c;
    }
  } catch(e){}
  ...
  if (cached) {
    ...
    applyGeo('XX'); // default to NA until trace returns
    fetch('/cdn-cgi/trace').then(function(r){return r.text();}).then(function(t){
      var m = t.match(/loc=(\w+)/);
      var country = m ? m[1] : 'XX';
      try { localStorage.setItem('pwp-geo', JSON.stringify({c:country, t:Date.now()})); } catch(e){}
      applyGeo(country);
