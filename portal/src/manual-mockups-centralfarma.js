...
<link rel="stylesheet" href="https://fonts.googleapis.com/css2?family=Fraunces:opsz,wght@9..144,500;9..144,600;9..144,700&family=Inter:wght@400;500;600;700;800;900&display=swap">
...
.brand-mark{width:38px;height:38px;flex:0 0 38px;border-radius:50%;background:var(--brand);display:flex;align-items:center;justify-content:center;color:#fff;font-family:var(--sans);font-weight:900;font-size:.92rem;letter-spacing:-.04em;box-shadow:var(--shadow-sm);text-transform:uppercase}
...
.hero-graphic .slide.is-active{opacity:1}
/* Subtle brand-green wash that ties stock photos back to the palette */
.hero-graphic::after{
  content:'';position:absolute;inset:0;pointer-events:none;
  background:linear-gradient(165deg, rgba(46,139,106,.10) 0%, rgba(14,42,77,.00) 35%, rgba(14,42,77,.18) 100%);
  z-index:2;
}
/* CF logo badge anchored bottom-left so brand presence stays through all three slides */
.hero-graphic .badge{
  position:absolute;left:1rem;bottom:1rem;z-index:3;
  display:inline-flex;align-items:center;gap:.55rem;
  padding:.4rem .75rem .4rem .4rem;background:rgba(255,255,255,.95);
  border-radius:99px;box-shadow:0 6px 14px rgba(14,42,77,.18);
  backdrop-filter:blur(8px);-webkit-backdrop-filter:blur(8px);
}
.hero-graphic .badge-mark{
  width:30px;height:30px;border-radius:50%;background:var(--brand);
  color:#fff;display:flex;align-items:center;justify-content:center;
  font-family:var(--sans);font-weight:900;font-size:.72rem;letter-spacing:-.04em;
}
.hero-graphic .badge-text{font-size:.72rem;font-weight:600;color:var(--navy);letter-spacing:.01em;padding-right:.25rem}
/* Slide caption pill · changes with active slide, gives visual rhythm */
.hero-graphic .caption{
  position:absolute;right:1rem;bottom:1rem;z-index:3;
  padding:.35rem .75rem;background:rgba(14,42,77,.78);color:#fff;
...
.hero-graphic .caption.is-active{opacity:1;transform:translateY(0)}

/* Slide overlays · per-slide composited elements (CF badge on delivery crate, fake clean license plate). Shown only when their target slide is active. */
.hero-graphic .slide-overlay{
  position:absolute;z-index:3;pointer-events:none;
  opacity:0;transition:opacity .35s ease;
}
.hero-graphic .slide-overlay.is-active{opacity:1}

.hero-graphic .crate-badge{
  left:30%;top:62%;
  width:14%;aspect-ratio:1/1;
  background:var(--brand);color:#fff;
  border-radius:50%;border:3px solid #fff;
  box-shadow:0 6px 16px rgba(0,0,0,.32), inset 0 -3px 8px rgba(0,0,0,.12);
  display:flex;align-items:center;justify-content:center;
  font-family:var(--sans);font-weight:900;font-size:clamp(.7rem, 2.1vw, 1.4rem);
  letter-spacing:-.04em;
  transform:translate(-50%,-50%) rotate(-6deg);
  transform-origin:center;
}

.hero-graphic .plate-cover{
  left:27%;top:87%;
  width:22%;height:4.2%;
  background:linear-gradient(180deg,#F5D013 0%,#E8C108 100%);
  display:flex;align-items:center;justify-content:center;
  font-family:'Inter',sans-serif;font-weight:800;
  font-size:clamp(.42rem,1.25vw,.82rem);
  color:#1a1a1a;letter-spacing:.04em;
  transform:translate(-50%,-50%) skewY(-1.5deg);
  border-radius:2px;
  box-shadow:0 1px 2px rgba(0,0,0,.28), inset 0 1px 0 rgba(255,255,255,.35);
}
/* Honor reduced-motion · disable crossfade, just show first image */
@media (prefers-reduced-motion: reduce){
  .hero-graphic .slide{transition:none}
  .hero-graphic .slide:not(:first-child){display:none}
  .hero-graphic .slide:first-child{opacity:1}
  .hero-graphic .caption{opacity:1;transform:none}
}
...
/* ─── Chat widget · talks to mockups.pymewebpro.com/api/chat/central-farma-drogueria ─── */
.chat-fab{
...
.chat-panel{
  ...
  box-shadow:0 24px 60px rgba(14,42,77,.22), 0 8px 16px rgba(14,42,77,.12);
  display:flex;flex-direction:column;overflow:hidden;
  border:1px solid var(--line);
  transform:translateY(20px);opacity:0;pointer-events:none;
  transition:transform .25s ease, opacity .25s ease;
}
.chat-panel[data-open="true"]{transform:translateY(0);opacity:1;pointer-events:auto}

.chat-header{
  display:flex;align-items:center;justify-content:space-between;gap:.75rem;
  padding:.95rem 1.1rem;background:var(--brand);color:#fff;
}
.chat-header .who{display:flex;align-items:center;gap:.65rem}
.chat-header .avatar{width:34px;height:34px;border-radius:50%;background:rgba(255,255,255,.2);display:flex;align-items:center;justify-content:center;font-family:var(--sans);font-weight:900;font-size:.78rem;letter-spacing:-.04em}
.chat-header .title{font-weight:600;font-size:.95rem;line-height:1.15}
.chat-header .subtitle{font-size:.72rem;opacity:.85;display:flex;align-items:center;gap:.35rem}
.chat-header .subtitle::before{content:'';width:7px;height:7px;border-radius:50%;background:#7CF7C5;box-shadow:0 0 0 2px rgba(124,247,197,.25)}
.chat-close{
  background:transparent;color:rgba(255,255,255,.85);border:none;padding:.3rem;
...
.chat-body::-webkit-scrollbar-thumb{background:rgba(20,43,61,.14);border-radius:3px}

.bubble{
  max-width:80%;padding:.7rem .9rem;border-radius:14px;
  font-size:.92rem;line-height:1.45;white-space:pre-wrap;word-wrap:break-word;
}
.bubble.bot{
  background:#fff;color:var(--ink);align-self:flex-start;
...
.bubble.typing{display:inline-flex;gap:3px;align-items:center;padding:.85rem 1rem;color:var(--ink-mute)}
.bubble.typing span{width:6px;height:6px;border-radius:50%;background:var(--ink-mute);animation:tdot 1.2s infinite ease-in-out}
...
.bubble.error{background:#FEF2F2;color:#991B1B;border:1px solid #FEE2E2;align-self:flex-start;border-bottom-left-radius:4px}

.wa-cta{
  display:inline-flex;align-items:center;gap:.5rem;
  align-self:flex-start;margin-top:.2rem;
  padding:.7rem 1rem;background:#25D366;color:#fff;border-radius:10px;
  text-decoration:none;font-weight:600;font-size:.88rem;
  box-shadow:0 6px 14px rgba(37,211,102,.32);
  transition:transform .15s ease;
}
.wa-cta:hover{transform:translateY(-1px);color:#fff}
.wa-cta svg{width:16px;height:16px}

.chat-input-row{
  display:flex;gap:.5rem;padding:.75rem;background:#fff;border-top:1px solid var(--line);
}
.chat-input{
  flex:1;border:1px solid var(--line);border-radius:10px;padding:.6rem .85rem;
  font:inherit;font-size:.92rem;color:var(--ink);outline:none;background:#FAFBFB;
  transition:border-color .15s ease, background .15s ease;
}
.chat-input:focus{border-color:var(--brand);background:#fff}
.chat-send{
  background:var(--brand);color:#fff;border:none;border-radius:10px;
  padding:0 .9rem;display:flex;align-items:center;justify-content:center;
  cursor:pointer;transition:background .15s ease;
}
.chat-send:hover{background:var(--brand-dark)}
.chat-send:disabled{background:var(--ink-mute);cursor:not-allowed}
.chat-send svg{width:18px;height:18px}

.chat-foot{
  padding:.45rem .75rem;background:#fff;border-top:1px solid var(--line);
  font-size:.7rem;color:var(--ink-mute);text-align:center;letter-spacing:.02em;
}

@media (max-width: 500px){
  .chat-panel{bottom:0;right:0;width:100vw;height:100vh;max-height:100vh;border-radius:0;border:none}
...
<section class="hero">
  <div class="wrap hero-grid">
    ...
    <figure class="hero-graphic" id="heroGallery" aria-label="Central Farma · escenas del día a día">
      <img class="slide is-active" src="./hero-pharmacist.jpg" alt="Farmaceuta de Central Farma atendiendo a un cliente con una sonrisa" width="800" height="1000" decoding="async" fetchpriority="high">
      <img class="slide" src="./hero-delivery.jpg" alt="Domiciliario de Central Farma en moto con caja entregando un pedido" width="800" height="1000" decoding="async" loading="lazy">
      <img class="slide" src="./hero-injection.jpg" alt="Aplicación profesional de una inyección en Central Farma" width="800" height="1000" decoding="async" loading="lazy">
      <div class="badge" aria-hidden="true">
        <span class="badge-mark">CF</span>
        <span class="badge-text">Central Farma</span>
      </div>
      <span class="caption is-active" data-i="0">Atención que se siente cercana</span>
      <span class="caption" data-i="1">Domicilios gratis en tu barrio</span>
      <span class="caption" data-i="2">Inyectología profesional</span>
      <!-- Per-slide overlays · positioned over the delivery photo to brand the crate and clean up the AI-garbled license plate. -->
      <span class="slide-overlay crate-badge" data-slide="1" aria-hidden="true">CF</span>
      <span class="slide-overlay plate-cover" data-slide="1" aria-hidden="true">FAR42M</span>
    </figure>
  </div>
</section>
...
<button class="chat-fab" id="chatOpenBtn" aria-label="Hablar con el asistente" aria-expanded="false" aria-controls="chatPanel">
  <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><path d="M21 11.5a8.38 8.38 0 0 1-.9 3.8 8.5 8.5 0 0 1-7.6 4.7 8.38 8.38 0 0 1-3.8-.9L3 21l1.9-5.7a8.38 8.38 0 0 1-.9-3.8 8.5 8.5 0 0 1 4.7-7.6 8.38 8.38 0 0 1 3.8-.9h.5a8.48 8.48 0 0 1 8 8v.5z"/></svg>
</button>

<aside class="chat-panel" id="chatPanel" role="dialog" aria-labelledby="chatTitle" aria-hidden="true">
  <header class="chat-header">
    <div class="who">
      <div class="avatar" aria-hidden="true">CF</div>
      <div>
        <div class="title" id="chatTitle">Asistente Central Farma</div>
        <div class="subtitle">en línea</div>
      </div>
    </div>
    <button class="chat-close" id="chatCloseBtn" aria-label="Cerrar chat">
  ...
  <form class="chat-input-row" id="chatForm" autocomplete="off">
    <input type="text" class="chat-input" id="chatInput" placeholder="Escribe tu mensaje…" aria-label="Mensaje" maxlength="500" required />
    <button type="submit" class="chat-send" id="chatSendBtn" aria-label="Enviar">
      <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.2" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><path d="M22 2 11 13M22 2l-7 20-4-9-9-4 20-7z"/></svg>
    </button>
  </form>
  <div class="chat-foot">Atendido por IA · siempre puedes hablar con un humano por WhatsApp</div>
</aside>
...
// ─── Hero gallery · 2s crossfade rotation, hover-pause, reduced-motion aware ───
(function(){
  var gallery = document.getElementById('heroGallery');
  if (!gallery) return;
  var slides = Array.prototype.slice.call(gallery.querySelectorAll('.slide'));
  var captions = Array.prototype.slice.call(gallery.querySelectorAll('.caption'));
  var overlays = Array.prototype.slice.call(gallery.querySelectorAll('.slide-overlay'));
  if (slides.length < 2) return;
  // Show slide-2 overlays on initial render (delivery crate badge + clean plate)
  // even before the first tick, so first impression of the delivery slide is correct.
  function syncOverlays(n){
    overlays.forEach(function(o){
      o.classList.toggle('is-active', parseInt(o.dataset.slide, 10) === n);
    });
  }
  // Respect user pref
  var motionOK = !window.matchMedia('(prefers-reduced-motion: reduce)').matches;
  if (!motionOK){ syncOverlays(0); return; }

  var i = 0;
  var timer = null;
  function show(n){
    slides.forEach(function(s, idx){ s.classList.toggle('is-active', idx === n); });
    captions.forEach(function(c, idx){ c.classList.toggle('is-active', idx === n); });
    syncOverlays(n);
  }
  function tick(){
    i = (i + 1) % slides.length;
...
// ─── Chat widget ───
(function(){
  var SLUG = 'central-farma-drogueria';
  var ENDPOINT = '/api/chat/' + SLUG;
  var WA_FALLBACK = 'https://wa.me/573023877739?text=' + encodeURIComponent('Hola Central Farma, tengo una pregunta');

  var fab = document.getElementById('chatOpenBtn');
  ...
  function addWaCta(url, label){
    var a = document.createElement('a');
    a.className = 'wa-cta';
    a.href = url;
    a.target = '_blank';
    a.rel = 'noopener noreferrer';
    a.innerHTML = '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.2" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><path d="M21 11.5a8.38 8.38 0 0 1-.9 3.8 8.5 8.5 0 0 1-7.6 4.7 8.38 8.38 0 0 1-3.8-.9L3 21l1.9-5.7a8.38 8.38 0 0 1-.9-3.8 8.5 8.5 0 0 1 4.7-7.6 8.38 8.38 0 0 1 3.8-.9h.5a8.48 8.48 0 0 1 8 8v.5z"/></svg><span></span>';
    a.querySelector('span').textContent = label;
    body.appendChild(a);
    body.scrollTop = body.scrollHeight;
  }
  function greet(){
    addBubble('bot', '¡Hola! Soy el asistente de Central Farma. Te ayudo con domicilios, inyectología, productos, o cualquier consulta. ¿En qué te puedo ayudar?');
  }
  ...
  async function send(text){
    ...
    try {
      ...
      if (!res.ok){
        var detail = '';
        try { var j = await res.json(); detail = j.error || ''; } catch(e){}
        var msg = res.status === 503
          ? 'El asistente está temporalmente fuera de línea. Habla directamente con nosotros por WhatsApp.'
          : 'Hmm, algo no salió bien. Escríbenos directamente por WhatsApp y te atendemos al instante.';
        addBubble('bot', msg, { error:true });
        addWaCta(WA_FALLBACK, 'Hablar por WhatsApp');
        return;
      }
      ...
      if (data.wa_link){
        var label = data.mode === 'order' ? 'Confirmar pedido por WhatsApp' : 'Continuar por WhatsApp';
        addWaCta(data.wa_link, label);
      }
      history.push({ role:'assistant', content: reply });
    } catch(err) {
      typing.remove();
      addBubble('bot', 'No pude enviar tu mensaje. Habla con nosotros por WhatsApp y te atendemos de una.', { error:true });
      addWaCta(WA_FALLBACK, 'Hablar por WhatsApp');
    } finally {
